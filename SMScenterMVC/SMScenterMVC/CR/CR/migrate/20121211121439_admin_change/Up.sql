...
2012-12-11  R0068      Толшин Кирилл     Подправил процедуру выборки action'ов
******************************************************************************/
...
procedure pr_group_update(id_group_ in number, name_group_ in varchar2, id_updater_ in number, id_branch_ in number);
...
procedure pr_actions_get(id_role_ in number, id_action_ in number, name_action_ in varchar2, description_action_ in varchar2, actions_user_id_ in number, order_by_ in varchar2, t_list out ref_cursor);
...
is
  ...
  begin
     if id_group_ is not null and name_group_ is not null and id_updater_ is not null  and id_updater_ is not null
         then update GROUPS set NAME=name_group_, USER_ID=id_updater_, BRANCHE_ID=id_branch_, DATE_MODIFY=SYSDATE WHERE ID=id_group_;
         commit;
     else
...
is
  ...
  user_branch_id number;
  begin
     sql_:='select ID, NAME, PHONE, DESCRIPTION, EMAIL, BRANCHE_ID, BRANCHE_NAME, USER_ID, USER_NAME from v_abonents where 1=1';
     ...
     sql_:=sql_||' group by ID, NAME, PHONE, DESCRIPTION, EMAIL, BRANCHE_ID, BRANCHE_NAME, USER_ID, USER_NAME ';
     if order_by_ is not null
        then sql_:=sql_||' order by '||order_by_;
     else
        sql_:=sql_||' order by lower(NAME)';
...
is
  ...
  begin
     ...
     if actions_user_id_ is not null
        then sql_:=sql_||' and ID in (select t.action_id from user_action_link t where t.user_id='||actions_user_id_||')';
     end if;
