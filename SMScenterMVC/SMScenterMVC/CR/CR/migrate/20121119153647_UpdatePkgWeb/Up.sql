...
procedure pr_abonent_update(id_abonent_ in number, name_abonent_ in varchar2, phone_abonent_ in varchar2, description_ in varchar2, branch_id_ in number, id_updater_ in number);
...
is
  ...
  begin
     if name_abonent_ is not null and phone_abonent_ is not null and description_ is not null and id_creater_ is not null
         then INSERT INTO ABONENTS(ID, NAME, PHONE, DESCRIPTION, BRANCHE_ID, USER_ID, DATE_MODIFY) VALUES(s_abonents.nextval, name_abonent_, phone_abonent_, description_, (select u.branche_id from users u where u.id = id_creater_), id_creater_, sysdate);
         commit;
     else
         raise error_some_param_empty;
     end if;
...
is
  ...
  begin
     if id_abonent_ is not null and name_abonent_ is not null and phone_abonent_ is not null and description_ is not null and id_updater_ is not null
         then UPDATE ABONENTS SET NAME=name_abonent_, PHONE=phone_abonent_, DESCRIPTION=description_, BRANCHE_ID = branch_id_, USER_ID=id_updater_, DATE_MODIFY=sysdate WHERE ID = id_abonent_;
         commit;
     else
         raise error_some_param_empty;
     end if;
