...
is
  ...
  begin
     if id_abonent_ is not null and name_abonent_ is not null and phone_abonent_ is not null and description_ is not null and id_updater_ is not null
         then UPDATE ABONENTS SET NAME=name_abonent_, PHONE=phone_abonent_, DESCRIPTION=description_, USER_ID=id_updater_, DATE_MODIFY=sysdate WHERE ID = id_abonent_;
         commit;
     else
         raise error_some_param_empty;
     end if;
