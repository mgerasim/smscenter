...
2012-06-15  R0046      Михаил Герасимов  Отложенная отправка
******************************************************************************/

PROCEDURE get_messages(p_limit IN PLS_INTEGER, p_msgs OUT MSG_CURSOR)
IS
BEGIN
     OPEN p_msgs FOR SELECT * FROM (SELECT * FROM MESSAGES WHERE status_id=1 and started < current_timestamp ORDER BY priority ASC, CREATED_AT DESC) WHERE rownum  < p_limit+1 ;
END;
