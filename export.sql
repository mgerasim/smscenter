...
prompt =======================
prompt
create table SMSCENTER.BRANCHES
(
  ID     NUMBER not null,
...
comment on table SMSCENTER.BRANCHES
  is 'Филиалы';
alter table SMSCENTER.BRANCHES
...
prompt =========================
prompt
create table SMSCENTER.USER_ROLES
(
  ID          NUMBER not null,
  NAME        VARCHAR2(50),
...
alter table SMSCENTER.USER_ROLES
...
alter table SMSCENTER.USERS
  ...
  references SMSCENTER.USER_ROLES (ID);

prompt
prompt Creating table ABONENTS
prompt =======================
prompt
create table SMSCENTER.ABONENTS
(
  ID          NUMBER not null,
...
comment on table SMSCENTER.ABONENTS
  is 'Справочник телефонов';
alter table SMSCENTER.ABONENTS
...
alter table SMSCENTER.ABONENTS
  ...
  references SMSCENTER.BRANCHES (ID);
alter table SMSCENTER.ABONENTS
  add constraint FK_USERS_ABONENTS foreign key (USER_ID)
  references SMSCENTER.USERS (ID);

prompt
prompt Creating table ABONENT_GROUP_LINK
prompt =================================
prompt
create table SMSCENTER.ABONENT_GROUP_LINK
(
  ID         NUMBER,
  GROUP_ID   NUMBER,
  ABONENT_ID NUMBER
...
prompt ==============================
prompt
create table SMSCENTER.ARCHIVE_MESSAGE
(
  ID         NUMBER(38) not null,
...
comment on table SMSCENTER.ARCHIVE_MESSAGE
  is 'Архив сообщений';
comment on column SMSCENTER.ARCHIVE_MESSAGE.TASK_ID
  is '0 - ??????? ?? ?????????';
comment on column SMSCENTER.ARCHIVE_MESSAGE.STATUS_ID
  is '1 - ?????? - ?????? ????????';
comment on column SMSCENTER.ARCHIVE_MESSAGE.MESSAGE
  is '????? ??? ?????????';
alter table SMSCENTER.ARCHIVE_MESSAGE
...
comment on table SMSCENTER.ERRORS
  is '???? ?????? ??? ???????? ???';
alter table SMSCENTER.ERRORS
...
comment on column SMSCENTER.GROUPS.IS_DELETE
  is 'Признак того, что группа рассылки удалена, временное поле';
alter table SMSCENTER.GROUPS
...
prompt =========================
prompt
create table SMSCENTER.STATUS_MSG
(
  ID   NUMBER not null,
  NAME VARCHAR2(100)
)
...
alter table SMSCENTER.STATUS_MSG
...
prompt ========================
prompt
create table SMSCENTER.TYPE_TASK
(
  ID       NUMBER not null,
  NAME     VARCHAR2(50),
  PRIORITY NUMBER
...
comment on table SMSCENTER.TYPE_TASK
  is '???? ????? ? ?? ?????????';
comment on column SMSCENTER.TYPE_TASK.ID
  is '?????????? ?????????????';
comment on column SMSCENTER.TYPE_TASK.NAME
  is '??? ??????';
comment on column SMSCENTER.TYPE_TASK.PRIORITY
  is '?????????, ??? ?????? ???????? ??? ?????? ?????????';
alter table SMSCENTER.TYPE_TASK
...
comment on column SMSCENTER.TASKS.GROUP_MSG
  is '??????? ?? ???????? ?????????? ??? ????? ????? ?????????. 0 - ? messages, 1-? tasks';
alter table SMSCENTER.TASKS
  add primary key (ID)
  using index 
  tablespace SMSCENTER
  pctfree 10
...
alter table SMSCENTER.TASKS
  ...
  references SMSCENTER.TYPE_TASK (ID);
alter table SMSCENTER.TASKS
...
prompt =======================
prompt
create table SMSCENTER.MESSAGES
(
  ID         NUMBER(38) not null,
  PHONE      VARCHAR2(255 CHAR),
  CREATED_AT DATE default sysdate,
...
comment on column SMSCENTER.MESSAGES.TASK_ID
  is '0 - ??????? ?? ?????????';
comment on column SMSCENTER.MESSAGES.STATUS_ID
  is '1 - ?????? - ?????? ????????';
comment on column SMSCENTER.MESSAGES.MESSAGE
  is '????? ??? ?????????';
alter table SMSCENTER.MESSAGES
  add primary key (ID)
  using index 
  tablespace SMSCENTER
  pctfree 10
...
prompt ========================
prompt
create table SMSCENTER.TEMPLATES
(
  ID NUMBER not null
)
tablespace SMSCENTER
  ...
  initrans 1
  maxtrans 255;
comment on table SMSCENTER.TEMPLATES
  is '??????????????, ??? ? ?????? ??????? ????? ??????? ?????????, ?? ??? ??????????? ????????? ??? ?? ??????.';
alter table SMSCENTER.TEMPLATES
...
prompt ================================
prompt
create table SMSCENTER.USER_BRANCHE_LINK
(
  ID         NUMBER,
  USER_ID    NUMBER,
  BRANCHE_ID NUMBER
...
tablespace SMSCENTER
  ...
  initrans 1
  maxtrans 255;
comment on table SMSCENTER.USER_BRANCHE_LINK
  is 'Привязка пользователей к филиалам';
alter table SMSCENTER.USER_BRANCHE_LINK
...
prompt ============================
prompt
create sequence SMSCENTER.S_ABONENTS
minvalue 1
...
prompt ======================================
prompt
create sequence SMSCENTER.S_ABONENT_GROUP_LINK
minvalue 1
maxvalue 9999999999999999999999999999
start with 173
increment by 1
...
prompt ==========================
prompt
create sequence SMSCENTER.S_ERRROR
minvalue 1
maxvalue 9999999999999999999999999999
start with 1
increment by 1
...
prompt ==========================
prompt
create sequence SMSCENTER.S_GROUPS
minvalue 1
maxvalue 9999999999999999999999999999
start with 55
increment by 1
...
prompt ============================
prompt
create sequence SMSCENTER.S_MESSAGES
minvalue 1
maxvalue 9999999999999999999999999999
start with 11728
increment by 1
...
prompt ==============================
prompt
create sequence SMSCENTER.S_STATUS_MSG
minvalue 1
maxvalue 9999999999999999999999999999
start with 21
increment by 1
...
prompt =========================
prompt
create sequence SMSCENTER.S_TASKS
minvalue 1
maxvalue 9999999999999999999999999999
start with 10198
increment by 1
...
prompt =============================
prompt
create sequence SMSCENTER.S_TYPE_TASK
minvalue 1
maxvalue 9999999999999999999999999999
start with 4
increment by 1
...
prompt =========================
prompt
create sequence SMSCENTER.S_USERS
minvalue 1
maxvalue 9999999999999999999999999999
start with 22
increment by 1
...
prompt ==============================
prompt
create sequence SMSCENTER.S_USER_ROLES
minvalue 1
maxvalue 9999999999999999999999999999
start with 21
increment by 1
...
prompt =============================
prompt
CREATE OR REPLACE VIEW SMSCENTER.UTV_RESULT_FULL AS
...
/
grant select, insert, update, delete, references on SMSCENTER.UTV_RESULT_FULL to PUBLIC;
...
prompt ==========================
prompt
CREATE OR REPLACE VIEW SMSCENTER.UTV_LAST_RUN AS
SELECT a.id utp_id, program, run_id last_run_id
    ...
    WHERE a.id = b.utp_id(+)
/
grant select, insert, update, delete, references on SMSCENTER.UTV_LAST_RUN to PUBLIC;
...
prompt ======================
prompt
create or replace view smscenter.vi_tasks as
...
prompt ==========================
prompt
create or replace package smscenter.PKG_ERROR is
/******************************************************************************			
...
prompt =========================
prompt
create or replace package smscenter.PKG_GATE is
...
prompt ========================
prompt
create or replace package smscenter.pkg_sms is
...
prompt ========================
prompt
create or replace package smscenter.PKG_WEB is
...
prompt ============================
prompt
create or replace package smscenter.UT_PKG_GATE is
...
prompt ================================
prompt
create or replace procedure smscenter.SMS_ARCHIVING is
 i integer;
begin
...
prompt ===============================
prompt
create or replace package body smscenter.PKG_ERROR is
/******************************************************************************			
...
prompt ==============================
prompt
create or replace package body smscenter.PKG_GATE is
/*******************************************************************************
  -- Автор  : H02-GERASIMOVMN
  -- Дата создания : 17.02.2012 14:22:47
  -- ОПИСАНИЕ:  Пакет для работы с заданиями на отправку SMS
...
prompt =============================
prompt
create or replace package body smscenter.pkg_sms is
...
---------------------------------------------------------------------------
  ...
  result_ varchar2(20);
  begin
    select s.name into result_ from smscenter.messages m
    left join smscenter.status_msg s on m.status_id=s.id
    where m.id=sms_id_;
    return result_;
...
---------------------------------------------------------------------------
  ...
  user_id number;
    begin
    select id into user_id from smscenter.users
...
prompt =============================
prompt
create or replace package body smscenter.PKG_WEB is
...
prompt =================================
prompt
create or replace package body smscenter.UT_PKG_GATE is
/*******************************************************************************
  ????:       ??????:  ?????:            ????????:
----------  -------  ----------------  ------------------------------------			
2012-02-21           ?????? ?????????  ???????? ????? ??????
